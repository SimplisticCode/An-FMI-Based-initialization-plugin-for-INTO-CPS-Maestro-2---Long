...
\begin{definition}[Initialization]\label{def:initialization}
  ...
  Fixed point is defined in \cref{def:fixedpoint} and
  loops in defined as the set of all non trivial SCC see \cref{def:loop}.
\end{definition}
...
An example of a co-simulation scenario where fixed point iteration ia needed can be seen in \cref{fig:fmu_cycle} where the Initialization Graph of the quarter car system from \cref{sec:case_study} is shown.
