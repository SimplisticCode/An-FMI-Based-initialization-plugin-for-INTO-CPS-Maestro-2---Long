...
\end{definition}
\claudio{The above definition should not be called Step, because it can be confused with co-simulation step. Maybe call it Operations? Note that in the sentence before the definition, you say that these defs only refer to the initialization.}
...
\claudio{I'm now confused by the above definition, as it's trying to use the step definition. But the step definition is the same as the INitialization definition....}
...
\claudio{The graph construction is similar to the one in \cite{Gomes2019c} except that the later focuses on the co-simulation step, while we here focus on the initialization phase.}
...
The following definitions will formalize \claudio{Simon, please avoid the future tense in the text. This is because the present tense is used to describe an action that is eternal... and the text in the paper is eternal.} the concept of an algebraic loop in a co-simulation scenario, and define the problem these algebraic loops are introducing.
...
\claudio{Using mathit tells latex that the text is a single block and avoids formatting problems. See \url{https://tex.stackexchange.com/questions/482743/typesetting-multi-letter-variable-names-in-math-mode}.}
